...
class SparkFieldComparator:
    """L4 hash-gated field comparison on unambiguous Spark record pairs."""
    ...
    def execute(
        ...
        matched_reconciliation = reconciliation.filter(
            F.col("reconciliation_status") == "MATCHED"
        )

        # A business key is only safe for L4 when it identifies exactly one
        # source row and exactly one target row. Duplicate-key populations are
        # ambiguous: hashing can tell whether two chosen rows are equal, but it
        # cannot prove which duplicate source row corresponds to which target
        # row. Keep those keys as evidence and never feed them into SHA-256 or
        ...
        unique_matched_reconciliation = matched_reconciliation.filter(
            (F.coalesce(F.col("_source_key_count"), F.lit(0)) == 1)
            & (F.coalesce(F.col("_target_key_count"), F.lit(0)) == 1)
        )
        pairs = unique_matched_reconciliation.select(
            "_s", "_t", "match_type", "match_key"
        )
        ...
        hash_summary = hashed_pairs.agg(
            F.sum(
                F.when(
                    F.col("__source_row_hash") == F.col("__target_row_hash"), 1
                ).otherwise(0)
            ).alias("hash_equal"),
            F.sum(
                F.when(
                    ~F.col("__source_row_hash").eqNullSafe(F.col("__target_row_hash")),
                    1,
                ).otherwise(0)
            ).alias("hash_changed"),
        ).first()
        ...
        eligible_records = hash_equal + hash_changed
        ambiguous_duplicate_key_count = duplicate_pairs.count()
        comparison_pairs = hashed_pairs.filter(
            ~F.col("__source_row_hash").eqNullSafe(F.col("__target_row_hash"))
        )
        ...
        for source_column, target_column, mapping in resolved_pairs:
            ...
            is_numeric_field = isinstance(
                source_types.get(source_column), NumericType
            ) and isinstance(target_types.get(target_column), NumericType)
        ...
        count_row = (
            comparison_pairs.agg(
                F.count(F.lit(1)).alias("hash_changed_record_count"),
                *[
                    F.sum(F.when(bad, 1).otherwise(0))
                    .cast("long")
        ...
        primary_matched_records = int(match_counts.get("primary_matched_count") or 0)
        compared = eligible_records * len(comps)
        mismatch_rows = F.array(
            *[
                ...
                comparison_type,
                tolerance_value,
                tolerance_type in comps
            ]
        )
        field_mismatches = (
            comparison_pairs.select(F.explode(mismatch_rows).alias("mismatch"))
            .filter(F.col("mismatch").isNotNull())
            .select("mismatch.*")
            if comps
            else None
        )
        evidence_summary = (
            field_mismatches.agg(
                F.countDistinct("key").alias("records_with_mismatch")
            ).first()
            if field_mismatches is not None
            else None
        )
        records_with_mismatch = (
            int(evidence_summary["records_with_mismatch"] or 0)
            ...
            return {
                "metrics": {
                    ...
                    "field_conformity_pct": safe_rate_pct(
                        compared - total_mismatch, compared, zero_value=100.0
                    ),
                    "field_mismatch_rate_pct": safe_rate_pct(
                        total_mismatch, compared
                    ),
                    ...
                    "effective_column_mappings": [
                        {
                            ...
                            "normalization": dict(
                                (mapping or {}).get("normalization") or {}
                            ),
                            "tolerance": (mapping or {}).get("tolerance"),
                            "tolerance_pct": (mapping or {}).get("tolerance_pct"),
                            "comparison_type": (
                                "PERCENTAGE_TOLERANCE"
                                if (mapping or {}).get("tolerance_pct") is not None
                                else "NUMERIC_TOLERANCE"
                                if (mapping or {}).get("tolerance") is not None
                                else "EXACT"
                            ),
                        }
                        for source_column, target_column, mapping in resolved_pairs
                    ],
                    "field_mismatches": host._bounded(
                        field_mismatches, total_mismatch
                    ),
                    "duplicate_matched_pairs": host._bounded(
                        duplicate_pairs, ambiguous_duplicate_key_count
                    ),
                },
            }
        finally:
            try:
                duplicate_pairs.unpersist(blocking=False)
                hashed_pairs.unpersist(blocking=False)
            except Exception:
                pass
